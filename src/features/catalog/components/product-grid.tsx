...
export function ProductGrid({
  ...
  if (!items?.length) {
        ...
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed py-20 text-center px-4">
          ...
          {onClearFilters && (
            <button
              ...
              className="mt-4 rounded-xs bg-primary px-3 py-1.5 text-xs font-semibold text-primary-foreground hover:bg-primary/90 transition-colors"
            >
              Clear all filters
            </button>
          )}
    ...
    return (
      <div className="grid place-items-center rounded-xl border border-dashed py-24 text-sm text-muted-foreground">
        No products found.
      </div>
    );
  }
